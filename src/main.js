...
const TEAMS = {
  ...
  away: {
    ...
    color: '#dd0000', altColor: '#222222', gkColor: '#ff8800',
    formation: {
      GK:  { x: 0,    z: 48  },
      LB:  { x: 28,   z: 36  }, CB1: { x: 10,   z: 40  },
      CB2: { x: -10,  z: 40  }, RB:  { x: -28,  z: 36  },
...
const HOME_NAMES = ['Ter Stegen','Cancelo','Christensen','Kounde','Balde','De Jong','Pedri','Gavi','Raphinha','Lewandowski','Yamal'];
const AWAY_NAMES = ['Courtois','Carvajal','Rudiger','Militao','Mendy','Tchouameni','Camavinga','Modric','Vinicius','Bellingham','Rodrygo'];
...
function attemptAITackle(tackler, holder, tacklerIsHome) {
  ...
  if (Math.random() < 0.54) {
    ...
    if (holder === controlled) { cancelCharge(); showMsg('DISPOSSESSED!'); }
...
function setupAdjustPanel() {
  ...
  cbStadium.checked = true;
  cbStadium.disabled = true;
  if (vlStadium) vlStadium.textContent = 'ON';
...
// ============================================================
function setupUI() {
  document.getElementById('btn-quickmatch').onclick = startMatch;
  ...
  document.getElementById('btn-restart').onclick = startMatch;
  ...
  document.getElementById('btn-playagain').onclick = startMatch;

  // ── Snow Toggle Button ──────────────────────────────────────
  const btnSnow = document.createElement('button');
  btnSnow.textContent = 'SNOW: ON';
  // Style to match existing UI roughly (assuming simple CSS)
  btnSnow.style.cssText = "display:block; margin: 15px auto; padding: 10px 20px; font-size: 16px; background: rgba(255,255,255,0.15); color: white; border: 1px solid rgba(255,255,255,0.4); cursor: pointer; font-family: sans-serif; font-weight: bold; text-transform: uppercase; letter-spacing: 1px;";
  
  btnSnow.onclick = () => {
    snowActive = !snowActive;
    btnSnow.textContent = 'SNOW: ' + (snowActive ? 'ON' : 'OFF');
    if (_snowSys) _snowSys.visible = snowActive;
    // Update sky/fog for weather
    const col = snowActive ? 0xa8b8c4 : 0x88b8e8;
    scene.background = new THREE.Color(col);
    scene.fog = new THREE.FogExp2(col, snowActive ? 0.007 : 0.006);
  };

  const qmBtn = document.getElementById('btn-quickmatch');
  if (qmBtn && qmBtn.parentNode) qmBtn.parentNode.insertBefore(btnSnow, qmBtn.nextSibling);
}
